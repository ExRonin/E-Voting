 <?php
	class Admin extends CI_Controller
	{
		...
		public function updatedatapilketos()
		{
			...
			$update = $this->Admin_Model->updatedatapilketos($judul, $tapel, $tgl);
			if ($update = true) {
				$updateuser	= $this->Admin_Model->updateuser($judul);
		...
		public function updatedpt($username)
		{
			...
			$update		= $this->Admin_Model->updatedpt($username, $password, $nm_siswa, $jk, $kd_kelas, $hadir);
			if ($update = true) {
		...
		public function simpancalon()
		{
			...
			if ($this->upload->do_upload('photo')) {
				...
				$this->Admin_Model->tambahcalon($nisn, $no, $nama, $deskripsi, $photo);
		...
		public function updatecalon()
		{
			...
			$img_ext			= pathinfo($img, PATHINFO_EXTENSION);
			$photo				= $config['file_name'] . "." . $img_ext;
			$update		= $this->Admin_Model->updatecalon($nisn, $no, $nama, $deskripsi, $photo);
			if ($update = true) {
				$this->session->set_flashdata('info', 'Berhasil MemperbaruiData');
		...
		public function cetakdaftarhadir()
		{
			...
			$pdf->Cell(190, 7, 'Daftar Hadir Pemilihan ', 0, 1, 'C');
			$pdf->Cell(10, 7, '', 0, 1);
		...
		public function laporan()
		{
			...
			$pdf->SetFont('Arial', 'B', 16);
			//HEADER LAPORAN
			$pdf->Cell(290, 7, 'LAPORAN PELAKSANAAN ' . $data['judul'], 0, 1, 'C');
		...
		{
			$hapuspakeko = $this->db->truncate('tb_siswa');
			if ($hapuspakeko = true) {
				$this->session->set_flashdata('info', 'Berhasil Menghapus Data');
				redirect('admin/datadpt/');
			} else {
